...
- Windows 로컬 `ACE` Wake Word 감지와 음성 Orb 자동 호출
- 임시 녹음 자동 정리 및 사용자가 명시한 WAV 저장
...
- 별도 음성 명령 창과 승인 후 Registry에서 검증된 Windows 앱 실행·종료 요청
...
- **로컬 실행:** Windows App Paths·설치 위치에서 발견하고 검증한 앱을 이름과 별칭으로 실행·종료할 수 있습니다. 앱 실행은 항상 승인을 요구하며 임의 경로·인자·Shell 문자열·UAC 우회는 차단합니다.
